...
public class SetExitCommand implements Command {
    @Override
    ...
    public void onCommand(Player player, org.bukkit.command.Command command, String label, String... arguments) {
        ...
        if (arguments.length >= 2) {
            ...
            if (plugin.getArenaManager().arenaExists(arenaNome)) {
                player.sendMessage(FileManager.getMessage("arena_setsaida").replace("{0}", arenaNome));
                Arena arena = plugin.getArenaManager().getArena(arenaNome);
                arena.setExit(player.getLocation().add(0,1,0));
    ...
    public String getDescription() {
        return FileManager.getMessage("cmd_setsaida");
    }
}
